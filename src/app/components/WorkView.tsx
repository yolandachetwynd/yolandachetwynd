...
export default function WorkView({ collection, category, selectedIndex, setSelectedIndex}: WorkViewProps) {
    ...
    return(
        <div style={styles.wrapper}>
                        ...
                        {(selectedIndex === 0 || selectedIndex === collection.length -1) ?
                            <div style={styles.container}>
                                <div style={styles.button} onClick={()=>{ if (selectedIndex === collection.length - 1) { setSelectedIndex(selectedIndex-2)} else if (selectedIndex - 1 >= 0) {setSelectedIndex(selectedIndex - 1)} }}/>
                                    <img className="picture" style={styles.picture} src={`/yolandachetwynd.github.io/images/${work?.filename}`} />
                            
                                    {selectedIndex === collection.length -1 && 
                                        <img className="picture" style={styles.picture} src={`/yolandachetwynd.github.io/images/${collection[0]?.filename}`}/>
                            ...
                            <div style={styles.container}>
                                <div style={styles.button} onClick={()=>{ if (selectedIndex-2 >= 0 || selectedIndex -1 >=0){if (selectedIndex - 2 < 0) {setSelectedIndex(0)} else {setSelectedIndex(selectedIndex-2)}} }}/>
                                    <img className="picture" style={styles.picture} src={`/yolandachetwynd.github.io/images/${collection[selectedIndex]?.filename}`} usemap="#planetmap"/>
                                    ...
                                    <img className="picture" style={styles.picture} src={`/yolandachetwynd.github.io/images/${collection[selectedIndex+1]?.filename}`}/>
            ...
            {work?.additionalImages?.map((name: string) => {return <img key={name} src={`/yolandachetwynd.github.io/images/${name}`}/>}) }
        </div>
    )
}